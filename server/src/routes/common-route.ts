...
commonRoute.post("/api/sessions", async (c) => {
  try {
...
commonRoute.post("/api/sessions/oauth_state", async (c) => {
  try {
    const body = await c.req.json();
    const { state } = body;
...
commonRoute.post("/api/sessions/device_code", async (c) => {
  try {
    const body = await c.req.json();
...
// Update user profile
commonRoute.put("/api/users/:id", async (c) => {
  try {
...
commonRoute.post("/api/sessions/login_code", async (c) => {
  try {
    const body = await c.req.json();
